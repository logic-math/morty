...
stat_format_table() {
    ...
    # 当前执行状态
    echo -e "${BLUE}▸ 当前执行${NC}"
    echo -e "${TABLE_TOP_LEFT}$(printf '%*s' 62 '' | tr ' ' "$TABLE_HORIZONTAL")${TABLE_TOP_RIGHT}"
    ...
    if [[ "${CURRENT_JOB["exists"]}" == true ]]; then
        ...
        printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
            "模块" "${CURRENT_JOB["module"]:-N/A}"
        printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
            "Job" "${CURRENT_JOB["job"]:-N/A}"
        printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} ${status_color}%s %s${NC}\n" \
            "状态" "$status_icon" "${CURRENT_JOB["status"]:-N/A}"
        printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s/%s (%s%%)${NC}\n" \
            "进度" "${CURRENT_JOB["tasks_completed"]:-0}" "${CURRENT_JOB["tasks_total"]:-0}" "${CURRENT_JOB["progress"]:-0}"
        printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
            "循环次数" "${CURRENT_JOB["loop_count"]:-0}"
    else
        printf "${TABLE_VERTICAL} %-58s ${TABLE_VERTICAL}\n" "  无正在执行的 Job"
    fi
    echo -e "${TABLE_LEFT_T}$(printf '%*s' 62 '' | tr ' ' "$TABLE_HORIZONTAL")${TABLE_RIGHT_T}"

    # 上一个完成的 Job
    echo -e "${GRAY}▸ 上一个 Job${NC}"
    if [[ "${PREVIOUS_JOB["exists"]}" == true ]]; then
        printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
            "模块" "${PREVIOUS_JOB["module"]:-N/A}"
        printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
            "Job" "${PREVIOUS_JOB["job"]:-N/A}"
        printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s/%s${NC}\n" \
            "任务" "${PREVIOUS_JOB["tasks_completed"]:-0}" "${PREVIOUS_JOB["tasks_total"]:-0}"
        printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
            "耗时" "${PREVIOUS_JOB["elapsed"]:-N/A}"
    ...
    if [[ $issue_count -gt 0 ]]; then
        ...
        for issue in "${DEBUG_ISSUES[@]}"; do
            if [[ $display_count -lt 5 ]]; then
                ...
                printf "${TABLE_VERTICAL} ${RED}•${NC} %s${NC}\n" "$display_issue"
                ((display_count++))
            fi
        done
        if [[ $issue_count -gt 5 ]]; then
            printf "${TABLE_VERTICAL}   ${GRAY}... 还有 %d 个问题 ...${NC}\n" $((issue_count - 5))
        fi
    else
        printf "${TABLE_VERTICAL} %-58s ${TABLE_VERTICAL}\n" "  ${GREEN}✓${NC} 没有待解决的 debug 问题"
    fi
    echo -e "${TABLE_LEFT_T}$(printf '%*s' 62 '' | tr ' ' "$TABLE_HORIZONTAL")${TABLE_RIGHT_T}"
    ...
    echo -e "${GRAY}▸ 整体进度${NC}"
    printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
        "模块" "${STATUS_DATA["completed_modules"]:-0}/${STATUS_DATA["total_modules"]:-0}"
    printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
        "Jobs" "${STATUS_DATA["completed_jobs"]:-0}/${STATUS_DATA["total_jobs"]:-0}"
    printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s%%${NC}\n" \
        "完成度" "$PROGRESS_PERCENT"
    ...
    printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} ${GREEN}%s${GRAY}%s${NC} %s%%\n" \
        "进度条" "$bar_filled" "$bar_empty" "$PROGRESS_PERCENT"
    echo -e "${TABLE_LEFT_T}$(printf '%*s' 62 '' | tr ' ' "$TABLE_HORIZONTAL")${TABLE_RIGHT_T}"
    ...
    printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
        "累计时间" "$elapsed"
    printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
        "总循环" "${STATUS_DATA["total_loops"]:-0}"
    printf "${TABLE_VERTICAL} %-12s ${TABLE_VERTICAL} %s${NC}\n" \
        "版本" "${STATUS_DATA["version"]:-unknown}"
    echo -e "${TABLE_BOTTOM_LEFT}$(printf '%*s' 62 '' | tr ' ' "$TABLE_HORIZONTAL")${TABLE_BOTTOM_RIGHT}"
