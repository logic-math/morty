...
```json
{
  ...
  "context": {
    ...
    "current_job": {
      ...
      "tasks": ["Task 1: 创建 `internal/executor/engine.go` 文件结构","Task 2: 实现 `Engine` 接口","Task 3: 实现 `ExecuteJob(ctx, module, job)` 方法","Task 4: 实现前置条件检查","Task 5: 实现状态转换 (PENDING → RUNNING → COMPLETED/FAILED)","Task 6: 实现重试逻辑 (最多 3 次)","Task 7: 集成 Git 提交 (Job 完成后)","Task 8: 编写单元测试 `engine_test.go`"],
      "dependencies": [],
      "validator": "[ ] ExecuteJob 正确执行单个 Job"
    }
  }
}
```
