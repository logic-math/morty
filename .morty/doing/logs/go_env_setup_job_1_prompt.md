...
```json
{
  ...
  "context": {
    ...
    "current_job": {
      ...
      "tasks": ["Task 1: 检查操作系统类型 (`uname -s`)","Task 2: 检查架构类型 (`uname -m`)","Task 3: 安装 wget 或 curl（用于下载）","Task 4: 安装 tar（用于解压）","Task 5: 检查 git 版本 >= 2.0","Task 6: 安装 make（用于构建）","Task 7: 验证所有依赖可用"],
      "dependencies": [],
      "validator": "[ ] `uname` 命令返回正确结果"
    }
  }
}
```
...
**注意**: 系统通过检测输出中的  来判断任务成功，未检测到则标记为失败。
