...
```json
{
  ...
  "context": {
    ...
    "current_job": {
      ...
      "tasks": ["Task 1: 创建 `internal/cmd/stat.go` 文件","Task 2: 实现 `StatHandler` 结构体","Task 3: 实现参数解析 (`--watch`, `--json`)","Task 4: 检查 `.morty/status.json` 存在性","Task 5: 无状态时的友好提示","Task 6: 编写单元测试"],
      "dependencies": [],
      "validator": "[ ] 正确解析 `--watch` 和 `--json`"
    }
  }
}
```
