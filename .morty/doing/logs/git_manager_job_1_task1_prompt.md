...
**Job**: job_1
**当前 Task**: #1
**Task 描述**: 创建 `lib/git_manager.sh` 模块

## 任务列表

- [ ] 创建 `lib/git_manager.sh` 模块\n- [ ] 实现 `git_init_if_needed()`\n- [ ] 实现 `git_has_uncommitted_changes()`\n- [ ] 实现 `git_get_repo_root()`\n- [ ] 实现 `git_is_ignored(path)`\n
...
2. 执行当前 Task: 创建 `lib/git_manager.sh` 模块
3. 如有问题，记录 debug_log
4. 更新状态文件
5. 输出 RALPH_STATUS
