...
**Job**: job_3
**当前 Task**: #3
**Task 描述**: 实现 `version_get_loop_by_number(n)`

## 任务列表

- [ ] 实现 `version_reset_to_commit()`\n- [ ] 实现 `version_show_loop_history(n)`\n- [ ] 实现 `version_get_loop_by_number(n)`\n- [ ] 实现回滚前自动创建备份分支\n- [ ] 实现 `.morty/logs/` 目录保护（回滚时不删除）\n
...
2. 执行当前 Task: 实现 `version_get_loop_by_number(n)`
3. 如有问题，记录 debug_log
4. 更新状态文件
5. 输出 RALPH_STATUS
