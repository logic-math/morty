...
```json
{
  ...
  "context": {
    "completed_jobs_summary": ["cli/job_1: 完成 (5 tasks)"],
    "current_job": {
      "name": "job_2",
      "description": "Job execution",
      "tasks": ["创建 `morty_stat.sh` 脚本","实现 `stat_load_status()`","实现 `stat_load_logs()`","实现 `stat_get_current_job()`","实现 `stat_get_previous_job()`","实现 `stat_get_debug_issues()`","实现 `stat_get_progress()`","实现 `stat_get_elapsed_time()`","实现 `stat_format_table()`","实现 `stat_watch_mode()`"],
      "dependencies": [],
      "validator": "`morty stat` 应显示：当前执行、上一个 Job 摘要、Debug 问题、整体进度、累计时间"
    }
  }
}
```
...
**Job**: job_2
**当前 Task**: #1
**Task 描述**: 创建 `morty_stat.sh` 脚本

## 任务列表

- [ ] 创建 `morty_stat.sh` 脚本\n- [ ] 实现 `stat_load_status()`\n- [ ] 实现 `stat_load_logs()`\n- [ ] 实现 `stat_get_current_job()`\n- [ ] 实现 `stat_get_previous_job()`\n- [ ] 实现 `stat_get_debug_issues()`\n- [ ] 实现 `stat_get_progress()`\n- [ ] 实现 `stat_get_elapsed_time()`\n- [ ] 实现 `stat_format_table()`\n- [ ] 实现 `stat_watch_mode()`\n

## 验证器

- `morty stat` 应显示：当前执行、上一个 Job 摘要、Debug 问题、整体进度、累计时间\n- 当 `.morty/status.json` 不存在时，应提示 "请先运行 morty doing"\n- `-t` 选项应以表格形式输出（默认行为）\n- `-w` 选项应进入监控模式，每 60s 原地刷新一次\n- 应正确计算并显示整体进度百分比\n- 应显示当前 Job 的累计运行时间（格式：HH\n- 应显示上一个完成 Job 的摘要（模块/Job/状态/耗时）\n- 应显示当前 Job 需要 debug 的问题列表\n- Ctrl+C 应能优雅退出监控模式\n- 无\n

## 执行指令

请按照 Doing 模式的循环步骤执行：
1. 读取精简上下文了解当前状态
2. 执行当前 Task: 创建 `morty_stat.sh` 脚本
3. 如有问题，记录 debug_log
4. 更新状态文件
5. 输出 RALPH_STATUS
