...
```json
{
  ...
  "context": {
    "completed_jobs_summary": ["config/job_1: 完成 (6 tasks)","config/job_2: 完成 (8 tasks)","config/job_3: 完成 (8 tasks)","config/job_4: 完成 (9 tasks)","deploy/job_1: 完成 (7 tasks)","deploy/job_2: 完成 (7 tasks)","deploy/job_3: 完成 (7 tasks)","deploy/job_4: 完成 (8 tasks)","errors/job_1: 完成 (7 tasks)","git/job_1: 完成 (8 tasks)","git/job_2: 完成 (7 tasks)","git/job_3: 完成 (9 tasks)","go_env_setup/job_2: 完成 (7 tasks)","go_env_setup/job_3: 完成 (7 tasks)","go_env_setup/job_4: 完成 (5 tasks)","go_env_setup/job_5: 完成 (8 tasks)","parser/job_1: 完成 (7 tasks)","parser/job_2: 完成 (7 tasks)","parser/job_3: 完成 (6 tasks)","parser/job_4: 完成 (6 tasks)","parser/job_5: 完成 (6 tasks)","parser/job_6: 完成 (8 tasks)","parser/job_7: 完成 (7 tasks)"],
    "current_job": {
      "name": "job_1",
      "description": "Job execution",
      "tasks": ["Task 1: 创建 `internal/logging/logger.go` 定义 Logger 接口","Task 2: 创建 `internal/logging/slog_adapter.go` 实现 slog 适配器","Task 3: 实现所有日志级别方法 (Debug, Info, Warn, Error, Success, Loop)","Task 4: 支持结构化属性 (attrs)","Task 5: 实现 `WithContext` 添加上下文信息","Task 6: 实现 `WithJob` 添加 Job 上下文","Task 7: 实现 `SetLevel` 和 `GetLevel`","Task 8: 编写单元测试 `slog_adapter_test.go`"],
      "dependencies": [],
      "validator": "[ ] 各日志级别输出正确 (DEBUG, INFO, WARN, ERROR, SUCCESS, LOOP)"
    }
  }
}
```
