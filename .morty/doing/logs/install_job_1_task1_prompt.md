...
**Job**: job_1
**当前 Task**: #1
**Task 描述**: 创建 `lib/install.sh` 模块

## 任务列表

- [ ] 创建 `lib/install.sh` 模块\n- [ ] 实现 `install_check_deps()`\n- [ ] 实现 `install_check_bash_version()`\n- [ ] 实现 `install_check_git_version()`\n- [ ] 实现 `install_check_ai_cli()`\n- [ ] 实现 `install_check_optional_deps()`\n- [ ] 实现友好的缺失依赖提示和安装指导\n
...
2. 执行当前 Task: 创建 `lib/install.sh` 模块
3. 如有问题，记录 debug_log
4. 更新状态文件
5. 输出 RALPH_STATUS
