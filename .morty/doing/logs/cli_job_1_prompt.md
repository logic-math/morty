...
```json
{
  ...
  "context": {
    ...
    "current_job": {
      ...
      "tasks": ["Task 1: 创建 `internal/cli/parser.go` 文件","Task 2: 实现 `Parse(args []string) (*ParseResult, error)` 方法","Task 3: 支持长选项 (`--option`) 和短选项 (`-o`)","Task 4: 支持带值的选项 (`--option value` 或 `--option=value`)","Task 5: 支持布尔标志 (`--flag`)","Task 6: 支持位置参数","Task 7: 处理 `--` 终止选项解析","Task 8: 编写单元测试 `parser_test.go`"],
      "dependencies": [],
      "validator": "[ ] 正确解析命令名称"
    }
  }
