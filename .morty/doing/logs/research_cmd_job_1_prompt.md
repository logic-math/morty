...
```json
{
  ...
  "context": {
    ...
    "current_job": {
      ...
      "tasks": ["Task 1: 创建 `internal/cmd/research.go` 文件","Task 2: 实现 `ResearchHandler` 结构体","Task 3: 实现 `NewResearchHandler(cfg, logger)` 构造函数","Task 4: 实现 `Execute(ctx, args) error` 方法","Task 5: 解析命令行参数获取主题","Task 6: 检查并创建 `.morty/research/` 目录","Task 7: 编写单元测试 `research_test.go`"],
      "dependencies": [],
      "validator": "[ ] 无参数时交互式提示输入主题"
    }
  }
}
```
