...
**Tasks (Todo 列表)**:
- [x] Task 1: 实现 `watchMode()` 监控循环
- [x] Task 2: 设置刷新间隔（默认 60s）
- [x] Task 3: 实现屏幕清空（原地刷新）
- [x] Task 4: 信号处理（Ctrl+C 优雅退出）
- [x] Task 5: 刷新时重新收集数据
- [x] Task 6: 显示刷新时间
- [x] Task 7: 编写单元测试
...
**调试日志**:
- debug1: runWatchMode实现需要信号处理, 查看现有代码使用ctx.Done(), 猜想: 需要添加os/signal包来处理Ctrl+C, 验证: 实现signal.Notify捕获中断信号, 修复: 在runWatchMode中添加信号处理和优雅退出, 已修复
- debug2: 屏幕清空需要ANSI转义序列, 查看现有代码使用\\033[H\\033[2J, 猜想: 可能需要更完整的清屏序列, 验证: 添加\\033[2J\\033[3J\\033[H清除滚动缓冲区, 修复: 实现clearScreen方法使用多重ANSI序列, 已修复
- debug3: 需要添加刷新时间显示, 查看需求显示最后刷新时间, 猜想: 在watch header中添加时间显示, 验证: 实现displayWatchHeader方法显示刷新时间, 修复: 添加时间格式化显示, 已修复
