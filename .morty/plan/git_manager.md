...
**目标**: 实现安全的版本回滚，支持回滚到指定循环或提交

**前置条件**: Job 3 完成

**Tasks (Todo 列表)**:
- [ ] 实现 `git_reset_to_commit(commit_id)`: 回滚到指定提交
- [ ] 实现 `git_reset_to_loop(loop_number)`: 回滚到指定循环
- [ ] 实现回滚前备份当前状态（创建备份分支）
- [ ] 实现回滚后清理未跟踪文件（保留 .morty/logs/）
- [ ] 实现回滚确认和撤销机制

**验证器**:
- `git_reset_to_commit abc123` 应将 HEAD 重置到指定提交
- 回滚前应自动创建 `morty-backup-YYYYMMDD-HHMMSS` 分支
- 回滚后 `.morty/logs/` 目录应保留，其他未跟踪文件应被清理
- `git_reset_to_loop 3` 应找到 Loop #3 的提交并回滚到该状态
- 无效的回滚目标应被拒绝并返回错误信息
...
**Tasks (Todo 列表)**:
- [ ] 实现 `git_create_milestone(name)`: 为当前状态创建里程碑标签
- [ ] 实现 `git_list_milestones()`: 列出所有里程碑
- [ ] 实现 `git_checkout_milestone(name)`: 切换到里程碑状态
- [ ] 实现 `git_create_experiment(name)`: 创建实验分支
- [ ] 集成到 reset 命令

**验证器**:
- `git_create_milestone "v1.0-stable"` 应创建带注释的标签
- 里程碑标签应包含创建时间、描述和关联的循环编号
- `git_list_milestones()` 应按时间倒序列出所有里程碑
- 切换到里程碑应能正确恢复到该状态
- 实验分支应基于当前 HEAD 创建，并可独立演进
