...
- [x] git 提交中必须包含 install.sh 的安装脚本
...
**调试日志**:
- explore1: [探索发现] 项目使用 Go 1.21.6, 预编译二进制在 bin/morty (2.3MB), 安装目标为 ~/.morty/, 已记录
- debug1: install.sh 脚本创建完成, 支持 --force, --from-dist, --prefix 参数, 支持 .bashrc/.zshrc 自动配置, 已修复
...
**Tasks (Todo 列表)**:
- [x] Task 1: 检测当前版本（`morty version`）
- [x] Task 2: 获取最新版本信息（本地检查或远程查询）
- [x] Task 3: 版本对比，判断是否需要升级
- [x] Task 4: 备份当前版本二进制和配置
- [x] Task 5: 下载/编译新版本
- [x] Task 6: 安装新版本（调用 install.sh 逻辑）
- [x] Task 7: 验证升级成功
- [x] Task 8: 支持回滚（升级失败时恢复旧版本）
...
**调试日志**:
- explore1: [探索发现] 项目使用 ~/.morty/ 作为安装目录, 版本信息通过 JSON 格式输出 (morty -version), 已有 install.sh 和 build.sh 可用, 已记录
- debug1: upgrade.sh 创建完成, 支持 --check-only, --version, --offline, --force 参数, 实现了备份/回滚机制, 已修复
